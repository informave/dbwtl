...
CXXC_FIXTURE_TEST(OdbcTestbaseFixture, Connect)
{
	...
	stmt.prepare("SELECT CAST('foobar' as varchar(30)) UNION ALL SELECT 'bla';");
	stmt.execute();
...
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("SELECT ?, ?+1, 'foo' + ? + 'bar'");
	//stmt.prepare("SELECT ?, ?+1");
	CXXC_CHECK(stmt.paramCount() == 3);
	String a("foo");
	String b("bar");
	stmt.bind(1, Variant());
	stmt.bind(2, int(5));
	stmt.bind(3, String("hallo"));
	stmt.execute();
	...
	CXXC_CHECK( rs.column(2).get<int>() == 6 );
	CXXC_ECHO( rs.column(3).get<String>() );
	CXXC_CHECK( rs.column(3).get<String>() == String("foohallobar") );
...
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("SELECT CAST(-34.1234 as NUMERIC(8,6));");
	stmt.execute();
	DBMS::Resultset rs;
	rs.attach(stmt);
	rs.first();
...
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("SELECT cast('foobarbaz' as ntext)");
	stmt.execute();
	DBMS::Resultset rs;
	rs.attach(stmt);
	rs.first();
...
{
        DBMS::Statement stmt(dbc);
        stmt.prepare("INSERT INTO bigdata(mem) OUTPUT inserted.id VALUES(?)");
...
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CAST(? as NVARCHAR(15)), ?, ?, ?, ?, cast(? as varbinary(10));");
	stmt.bind(1, (signed int)(-34));
	stmt.bind(2, (unsigned int)(4192639274));
	stmt.bind(3, (signed short)(-92));
	stmt.bind(4, (unsigned short)(50273));
	stmt.bind(5, (signed long long)(-8390126317123926256));
	stmt.bind(6, (unsigned long long)(16012384751902387612));
	stmt.bind(7, (signed char)(-120));
	stmt.bind(8, (unsigned char)(204));
	stmt.bind(9, true);
	stmt.bind(10, Variant(bool(false)));
	stmt.bind(11, (float)(0.0007855));
	stmt.bind(12, (double)(12.00234987125));
	stmt.bind(13, TNumeric("-826.0028715"));
	stmt.bind(14, String("Unicode test: ÖÄÜ…"));
	//stmt.bind(15, TDate(2013, 3, 31));
	//stmt.bind(16, TTime(18, 59, 34));
	//stmt.bind(17, TTimestamp(2013, 3, 30, 17, 58, 33, 5000000));
	//stmt.bind(18, TVarbinary({0x03, 0x00, 0xf3, 0x85}));
	//
	for(int x = 15; x <= 18; ++x) stmt.bind(x, Variant());
	stmt.bind(17, TTimestamp(2013, 3, 30, 17, 58, 33, 0));
	stmt.bind(18, TVarbinary({0x03, 0x00, 0xf3, 0x85}));
	stmt.execute();
	DBMS::Resultset rs(stmt);
	rs.first();
	CXXC_ECHO( "Not output..." );
	CXXC_CHECK( rs.column(1).get<signed int>() == -34 );
	CXXC_CHECK( rs.column(2).get<unsigned int>() == 4192639274 );
	CXXC_CHECK( rs.column(3).get<signed short>() == -92 );
	CXXC_CHECK( rs.column(4).get<unsigned short>() == 50273 );
	//CXXC_CHECK( rs.column(5).get<signed long long>() == -8390126317123926256 );
	//CXXC_CHECK( rs.column(6).get<unsigned long long>() == 16012384751902387612 );
	...
	CXXC_CHECK( rs.column(11).get<float>() == float(0.0007855) );
	CXXC_CMP_DOUBLE( rs.column(12).get<double>(), double(12.00234987125), double(0.0000001) );
	
	CXXC_ECHO( rs.column(13).get<String>() );
	
	CXXC_CHECK( rs.column(13).get<TNumeric>() == TNumeric("-826.0028715") );
	CXXC_CHECK( rs.column(14).get<String>() == String("Unicode test: ÖÄÜ…") );
	//CXXC_CHECK( rs.column(15).get<TDate>() == TDate(2013, 3, 31) );
	//CXXC_CHECK( rs.column(16).get<TTime>() == TTime(18, 59, 34) );
	CXXC_CHECK( rs.column(17).get<TTimestamp>() == TTimestamp(2013, 3, 30, 17, 58, 33, 0) );
