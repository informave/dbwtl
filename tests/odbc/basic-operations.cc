...
CXXC_FIXTURE_TEST(OdbcTestbaseFixture, Connect)
{
	...
	stmt.prepare("SELECT 'foobar'::varchar(30) UNION ALL SELECT 'bla';");
	stmt.execute();
...
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("SELECT ?::int, ?+1, ('foo' || ? || 'bar')::varchar(30)");
...
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("SELECT 'foobarbaz'::text");
	stmt.execute();
	DBMS::Resultset rs;
	rs.attach(stmt);
	rs.first();
...
CXXC_FIXTURE_TEST(OdbcTestbaseFixture, NumericBind)
{
	DBMS::Statement stmt(dbc);
	stmt.prepare("INSERT INTO numtest(num1) VALUES(?);");
	stmt.bind(1, TNumeric("34.006"));
	stmt.execute();
}
...
CXXC_FIXTURE_TEST(OdbcTestbaseFixture, AllTypesBinding)
{
	...
	stmt.bind(13, TNumeric("826.00287156"));
	...
	CXXC_ECHO( rs.column(13).get<TNumeric>() );
	//CXXC_CHECK( rs.column(13).get<TNumeric>() == TNumeric("826.002871567218376") );
